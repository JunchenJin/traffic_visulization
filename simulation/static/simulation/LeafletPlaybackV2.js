...
L.Playback.MoveableMarker = L.Marker.extend({    
    ...
    //The function is added to update Icon based on attribute
    setIconColor:function(markerColor){
    	this.setIcon(
    ...
    getIconColor:function(markerColor){
    	return this.options.icon.options.markerColor;
    }
});
...
L.Playback.Track = L.Class.extend({
        ...
        initialize : function (geoJSON, options) {
			...
			// Codes added by Can Yang
			// tmod is time passing a tick time within a interval
			// _attributes is used to store the attributes for all ticks
			// attributed is used to store the attributes for timestamps
			this._attributes=[];
			var attributes=[]; 
			var currAttr;
			
			// Potential improvement can be to specify attribute in the options
			if(geoJSON.properties.hasOwnProperty('attribute')){						
				//The attribute are not for each tick, so it needs to be updated
				attributes = geoJSON.properties.attribute;
    	...
    	// the tick(timestamp) of Track object returns the coordinates of
    	// the vehicle at timestamp t
		...
		updateMarkerStyle : function(timestamp,options){
			console.log("UpdateMarkerStyle invoked");
			if (this._updateColor) {
				var previousColor = this._marker.getIconColor();
				var attr=this.attrAtTime(timestamp);
				var currentColor = options.legend[attr];
				
				if (currentColor!=previousColor) {
					this._marker.setIconColor(currentColor);
...
L.Playback.TrackController = L.Class.extend({
    ...
    tock : function (timestamp, transitionTime) {
        for (var i = 0, len = this._tracks.length; i < len; i++) {
            ...
            this._tracks[i].updateMarkerStyle(timestamp,this.options);
